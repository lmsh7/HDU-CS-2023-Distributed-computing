...
//LD_PRELOAD=./libdistributedmodel.so ./lab -l 2 -p 3 B1 B2 B3

void update_history(BalanceHistory *history, timestamp_t time, balance_t amount)
{
...
void parent_work(int count_nodes)
{
    ...
    /* 广播 STOP 消息*/
    Message msg;
    fill_message(&msg, STOP, get_physical_time(), NULL, 0);
    send_multicast(&msg);
    /* 收到所有 DONE 消息 */
    for (int i = 1; i < count_nodes; i++)
    {
...
void child_work(struct child_arguments args)
{
    ...
    char buf[BUF_SIZE];
    Message msg;
    timestamp_t current = get_physical_time(); // 是 lamport 逻辑时间
    ...
    while (true) {
        ...
        {   
            ...
            if (order->s_src == self_id) {
                ...
                update_history(&history, current, now_balance(&history) - order->s_amount);
              ...
              balance_t amount)
{
    ...
    /* 发送 TRANSFER 消息 */
    Message msg;
    fill_message(&msg, TRANSFER, get_physical_time(), &order, sizeof(order));
    send(src, &msg);
    /* 接受 dst 的 ACK 消息*/
